              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <h3 className="font-bold text-base truncate dark:text-white">{order.vendorName}</h3>
                  ...
                  <p className="font-bold text-wedding-blue text-sm mt-2">{order.amount}</p>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700 flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full bg-current ${order.status === 'active' ? 'text-emerald-500 animate-pulse' : order.status === 'pending' ? 'text-amber-500' : 'text-blue-500'}`}></span>
                    <span className={`dark:text-current text-sm font-bold`}>{order.status}</span>
                  </div>
                  <span className="text-[10px] text-gray-400 font-medium italic">
                    {order.status === 'pending' ? 'Menunggu konfirmasi Anda' : 'Terakhir diperbarui hari ini'}
                  </span>
                </div>
                <div className="flex gap-2">
                  <button className="flex-1 py-2 text-xs font-bold border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors dark:text-white">
                    {order.status === 'pending' ? 'Komplain' : 'Chat Vendor'}
                  </button>
                  <Link
                    href={order.status === 'pending' ? `/orders/${order.id}/release` : `/orders/${order.id}`}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg shadow-sm text-center flex items-center justify-center ${
                      order.status === 'pending' ? 'bg-amber-500 text-white' : 'bg-wedding-blue text-white'
                    }`}
                  >
                    {order.status === 'pending' ? 'Konfirmasi Kerja' : 'Detail Pesanan'}
                  </Link>
                </div>
              </div>
            </div>
